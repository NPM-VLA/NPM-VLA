...
python utils/convert_dataset_v21_to_v30.py \
    --src-repo-id=lerobot/pusht \
    --dst-repo-id=lerobot/pusht_v3
```
...
# ============================================================
# HELPER FUNCTIONS (missing from newer lerobot versions)
# ============================================================


def get_video_size_in_mb(video_path: Path) -> float:
...
def convert_dataset(
    ...
    convert_info(src_root, dst_root, data_file_size_in_mb, video_file_size_in_mb)
    convert_tasks(src_root, dst_root)
    episodes_metadata = convert_data(src_root, dst_root, data_file_size_in_mb)
    episodes_videos_metadata = convert_videos(src_root, dst_root, video_file_size_in_mb)
    convert_episodes_metadata(
        src_root, dst_root, episodes_metadata, episodes_videos_metadata
    )

    ds = LeRobotDataset(dst_repo_id)
    ds.push_to_hub()

    logging.info(
        f"✅ Converted {src_repo_id}@{revision} -> {dst_repo_id} (v3.0) and pushed to hub."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Source repository identifier on Hugging Face (v2.1 dataset): a community or a user name `/` the name of the dataset "
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Destination repository identifier on Hugging Face (v3.0 dataset): a community or a user name `/` the name of the dataset "
        "(e.g. `lerobot/pusht_v3`, `cadene/aloha_sim_insertion_human_v3`).",
    )
    parser.add_argument(
        "--branch",
        type=str,
